...
export function LemburDetailContent({ detail, lockAction, deleteAction }: { detail: LemburDetail; lockAction?: ReactNode; deleteAction?: ReactNode }) {
  return <div className="space-y-6">
    <section className="flex flex-col gap-4 rounded-xl border bg-card p-5 shadow-sm sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Detail lembur</p><h1 className="mt-2 text-2xl font-semibold tracking-tight">{detail.nama_kegiatan}</h1><div className="mt-3 flex flex-wrap items-center gap-3 text-sm text-muted-foreground"><span className="inline-flex items-center gap-1.5"><CalendarDays aria-hidden="true" className="size-4" />{formatLemburDate(detail.tanggal)}</span><span className="inline-flex items-center gap-1.5"><MapPin aria-hidden="true" className="size-4" />{detail.lokasi_kegiatan}</span></div></div><div className="flex shrink-0 flex-col items-start gap-3 sm:items-end"><StatusBadge status={detail.status} />{(detail.can_lock || detail.can_delete) && <div className="flex flex-wrap gap-2 sm:justify-end">{detail.can_lock ? lockAction : null}{detail.can_delete ? deleteAction : null}</div>}</div></section>
