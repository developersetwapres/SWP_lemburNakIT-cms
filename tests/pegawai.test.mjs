...
test("history uses its backend default size, supported filters, and employee route UUID", () => {
  ...
  const next = pegawaiDetailUrl(UUID, filters, { status: "complete" }); assert.match(next, /\/admin\/pegawai\/detail\?/); assert.equal(new URL(next, "https://cms.example.test").searchParams.get("uuid"), UUID); assert.doesNotMatch(next, /page=2/);
